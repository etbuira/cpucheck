...
static int check_item(void * const comp, void const * const config, void const * const table_element)
{
	...
	return !( c->byte_ex == elt->byte_ex
				&& c->word_ex == elt->word_ex
				&& c->word_exl == elt->word_exl
				&& c->word_exh == elt->word_exh
				&& c->dword_ex == elt->dword_ex
				&& c->dword_exl == elt->dword_exl
				&& c->dword_exh == elt->dword_exh
				&& c->qword_exl == elt->qword_exl
				&& c->qword_exh == elt->qword_exh );
